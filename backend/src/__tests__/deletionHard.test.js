 ...
 * Requires MONGODB_URI to be explicitly set — entire suite skipped in CI without a real DB.
 * This prevents beforeAll from even attempting a connection when no URI is available.
 */

const MONGODB_URI = process.env.MONGODB_URI;

/**
 * Skip conditions (all must be false to run):
 *  - MONGODB_URI is absent
 *  - MONGODB_URI points to localhost / 127.0.0.1 (unreachable in CI)
 *  - SKIP_DB_TESTS=true is explicitly set
 *
 * The test remains fully functional when a real remote MongoDB is available.
 */
function isRealDb(uri) {
  if (!uri) {
    return false;
  }
  ...
  // Skip if pointing at a local server (unreachable in CI)
...
const describeif = isRealDb(MONGODB_URI) ? describe : describe.skip;
