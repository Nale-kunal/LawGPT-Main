...
router.post('/create-subscription', requireAuth, createSubLimiter, async (req, res) => {
  try {
    ...
    if (caller?.securityFlags?.blocked) {
      ...
      return res.status(403).json({ error: 'ACCOUNT_BLOCKED', message: 'Your account has been suspended. Contact support.' });
    }
    ...
    if (!amountPaise) {
      return res.status(400).json({ error: 'INVALID_PRICE', message: 'No price defined for this plan' });
    }
    ...
    try {
      ...
      if (dbErr.code === 11000) {
        logger.warn({ userId }, 'create-subscription: DB unique index blocked duplicate active subscription');
        return res.status(409).json({
          error:   'SUBSCRIPTION_EXISTS',
          message: 'You already have an active or pending subscription.',
        });
...
router.post('/verify-payment', requireAuth, async (req, res) => {
  try {
    ...
    if (!razorpay_payment_id || !razorpay_subscription_id || !razorpay_signature) {
      return res.status(400).json({ error: 'MISSING_FIELDS', message: 'All three Razorpay fields are required' });
    }
    ...
    if (!subscription) {
      ...
      return res.status(403).json({ error: 'SUBSCRIPTION_MISMATCH', message: 'Subscription not found for your account' });
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!signature) {
      ...
      logPaymentEvent({ eventType: 'webhook.no_signature', status: 'rejected', rejectionReason: 'Missing signature header' });
    ...
    if (!signaturesMatch) {
      ...
      logPaymentEvent({ eventType: 'webhook.invalid_signature', status: 'rejected', rejectionReason: 'HMAC mismatch' });
    ...
    if (!event || typeof event !== 'object' || Array.isArray(event)) {
      return res.status(400).json({ error: 'MALFORMED_EVENT', message: 'Payload must be a JSON object' });
    }
    if (!event.event || !event.payload) {
      logger.warn({ keys: Object.keys(event) }, 'Webhook: missing event/payload fields');
      return res.status(400).json({ error: 'MALFORMED_EVENT', message: 'Missing event or payload fields' });
    }
    ...
    if (!ALLOWED_EVENTS.has(eventType)) {
      ...
      logPaymentEvent({ eventType, status: 'ignored', razorpayEventId: eventId, rawPayload: event, rejectionReason: 'Not in allowed event list' });
    ...
    if (event.created_at) {
      ...
      if (eventAgeMs > 5 * 60 * 1000) {
        ...
        logPaymentEvent({ eventType, status: 'late', razorpayEventId: eventId, rawPayload: event, rejectionReason: `Late event: age ${Math.round(eventAgeMs / 1000)}s` });
        logger.warn({ eventType, eventId, eventAgeMs }, 'Late webhook — processing anyway (idempotency guard active)');
      }
    }
    ...
    if (retryCount > 0) {
      logPaymentEvent({ eventType, status: 'retry', razorpayEventId: eventId, rawPayload: { retryCount }, rejectionReason: `Retry attempt ${retryCount}` });
    }
    if (retryCount > 5) {
    ...
    const webhookResult = await Promise.race([
      ...
      if (err.message === 'WEBHOOK_TIMEOUT') {
        ...
        inc('webhooks_error');
        logger.error({ err, eventType, eventId }, 'Webhook handler threw — event partially processed');
      }
    });
...
async function handleSubscriptionCharged(event, eventId, parsedPayload) {
  ...
  if (!rzpSubId) {
    logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, rawPayload: parsedPayload, rejectionReason: 'Missing subscription ID' });
    return;
  }
  ...
  if (!subscription) {
    ...
    logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, rawPayload: parsedPayload, rejectionReason: 'Not in DB', razorpaySubscriptionId: rzpSubId });
    return;
  }
  ...
  if (!lock) {
    logger.info({ rzpSubId, eventId }, 'subscription.charged: already processing (within TTL) — skipping');
    return;
  }
  ...
  try {
    ...
    if (notesUserId && notesUserId !== subscription.userId.toString()) {
      triggerAlert('ownership_mismatch', { notesUserId, dbUserId: subscription.userId.toString(), rzpSubId });
      await flagUserAbuse(subscription.userId.toString(), 50);
      logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: 'Ownership mismatch' });
      return;
    }
    ...
    if (paymentSubId && paymentSubId !== rzpSubId) {
      logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, rawPayload: parsedPayload, rejectionReason: 'payment.subscription_id mismatch' });
      return;
    }

    // ── 5. State machine check ───────────────────────────────────────────
    if (!ALLOWED_TRANSITIONS[subscription.status]?.includes('active')) {
      logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: `State machine: ${subscription.status} → active blocked` });
      return;
    }

    // ── 6. Amount tamper check ───────────────────────────────────────────
    if (amountPaise !== undefined && amountPaise !== null && amountPaise !== subscription.amountPaise) {
      triggerAlert('tampered_amount', { expected: subscription.amountPaise, received: amountPaise, rzpSubId });
      await flagUserAbuse(subscription.userId.toString(), 25);
      logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, razorpayPaymentId: rzpPayId, rawPayload: parsedPayload, rejectionReason: `Amount mismatch: expected ${subscription.amountPaise} got ${amountPaise}`, amountPaise });
      return;
    }
    ...
    try {
      ...
      if (!rzpSub || rzpSub.status !== 'active') {
        ...
        logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: `Razorpay sub status: ${rzpSub?.status}` });
        return;
      }
      if (rzpPayId) {
        const rzpPay = await rzp.payments.fetch(rzpPayId);
        if (rzpPay.status !== 'captured') {
          logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: `Payment not captured: ${rzpPay.status}` });
          return;
        }
        // ── 8. Customer binding validation (spec #7) ────────────────────
        if (subscription.customerId && rzpPay.customer_id && rzpPay.customer_id !== subscription.customerId) {
          triggerAlert('customer_mismatch', { paymentCustomerId: rzpPay.customer_id, subCustomerId: subscription.customerId, rzpSubId });
          await flagUserAbuse(subscription.userId.toString(), 50);
          logPaymentEvent({ eventType: 'subscription.charged', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: 'Customer ID mismatch' });
          return;
        }
      }
    } catch (apiErr) {
      // Razorpay may be temporarily unavailable — HMAC+state+idempotency guard
      logger.error({ apiErr, rzpSubId }, 'Razorpay API verify failed — proceeding with local guards');
    }
    ...
    if (idemResult.modifiedCount === 0) {
      ...
      logPaymentEvent({ eventType: 'subscription.charged', status: 'duplicate', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, rawPayload: parsedPayload });
      return;
    }
    ...
    try {
      await session.withTransaction(async () => {
        await Subscription.updateOne(
          { _id: subscription._id },
          { $set: { status: 'active', lastPaymentId: rzpPayId, failedAttempts: 0, currentPeriodStart: periodStart, currentPeriodEnd: periodEnd } },
          { session }
        );
        await activateSubscriptionPlan(
          subscription.userId.toString(), subscription.planType,
          subscription.billingCycle, subscription._id.toString(), periodEnd,
          { session }
        );
      });
    } finally {
    ...
    logPaymentEvent({ eventType: 'subscription.charged', status: 'processed', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, razorpayPaymentId: rzpPayId, rawPayload: parsedPayload, amountPaise: amountPaise ?? subscription.amountPaise });
    ...
    logger.info({ userId: subscription.userId.toString(), rzpSubId, planType: subscription.planType }, 'Plan activated via subscription.charged');
...
async function handlePaymentFailed(event, eventId, parsedPayload) {
  ...
  if (!rzpSubId) {
    logPaymentEvent({ eventType: 'payment.failed', status: 'ignored', razorpayEventId: eventId, rawPayload: parsedPayload, rejectionReason: 'No subscription_id' });
    return;
  }

  const subscription = await Subscription.findOne({ razorpaySubscriptionId: rzpSubId });
  if (!subscription) { logger.warn({ rzpSubId }, 'payment.failed: not in DB'); return; }
  ...
  if (idemResult.modifiedCount === 0) {
    logPaymentEvent({ eventType: 'payment.failed', status: 'duplicate', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload });
    return;
  }

  logPaymentEvent({ eventType: 'payment.failed', status: 'processed', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, rawPayload: parsedPayload });
  inc('payments_failed');
  inc('webhooks_processed');
  // Spec #5: notify user of failed payment
  notifyUser(subscription.userId.toString(), 'payment_failed', {});
  logger.warn({ userId: subscription.userId.toString(), rzpSubId }, 'payment.failed recorded (no downgrade — awaiting retry/halt)');
}
...
async function handleSubscriptionCancelled(event, eventId, parsedPayload, eventType) {
  ...
  if (!rzpSubId) {
    logPaymentEvent({ eventType, status: 'ignored', razorpayEventId: eventId, rawPayload: parsedPayload, rejectionReason: 'No subscription ID' });
    return;
  }

  const subscription = await Subscription.findOne({ razorpaySubscriptionId: rzpSubId });
  if (!subscription) { logger.warn({ rzpSubId, eventType }, 'Not in DB on cancel'); return; }
  ...
  if (!ALLOWED_TRANSITIONS[subscription.status]?.includes(newStatus)) {
    logPaymentEvent({ eventType, status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: `State machine: cannot transition ${subscription.status} → ${newStatus}` });
    return;
  }
  ...
  const idemResult = await Subscription.updateOne(
    { _id: subscription._id, processedEvents: { $ne: eventId } },
    { $push: { processedEvents: eventId }, $set: { status: newStatus, cancelledAt: new Date(), lastEventAt: new Date() } }
  );
  if (idemResult.modifiedCount === 0) {
    logPaymentEvent({ eventType, status: 'duplicate', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload });
    return;
  }
  ...
  try {
    await cancelSession.withTransaction(async () => {
      await cancelSubscriptionPlan(subscription.userId.toString(), subscription._id.toString(), { session: cancelSession });
    });
  } finally {
    await cancelSession.endSession();
  }
  logPaymentEvent({ eventType, status: 'processed', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, rawPayload: parsedPayload });
  inc('webhooks_processed');
  // Spec #5: notify user of cancellation
  notifyUser(subscription.userId.toString(), 'subscription_cancelled', {
    accessUntil: subscription.currentPeriodEnd,
  });
  logger.info({ userId: subscription.userId.toString(), rzpSubId, eventType }, 'Subscription cancelled — user reverted to free');
}
...
async function handleSubscriptionHalted(event, eventId, parsedPayload) {
  ...
  if (!rzpSubId) {
    logPaymentEvent({ eventType: 'subscription.halted', status: 'ignored', razorpayEventId: eventId, rawPayload: parsedPayload });
    return;
  }
  ...
  if (!ALLOWED_TRANSITIONS[subscription.status]?.includes('failed')) {
    logPaymentEvent({ eventType: 'subscription.halted', status: 'rejected', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload, rejectionReason: `State machine: cannot transition ${subscription.status} → failed` });
    return;
  }
  ...
  if (idemResult.modifiedCount === 0) {
    logPaymentEvent({ eventType: 'subscription.halted', status: 'duplicate', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, rawPayload: parsedPayload });
    return;
  }
  ...
  try {
    await haltSession.withTransaction(async () => {
      await cancelSubscriptionPlan(subscription.userId.toString(), subscription._id.toString(), { session: haltSession });
    });
  } finally {
    await haltSession.endSession();
  }
  logPaymentEvent({ eventType: 'subscription.halted', status: 'processed', razorpayEventId: eventId, subscriptionId: subscription._id, userId: subscription.userId, razorpaySubscriptionId: rzpSubId, rawPayload: parsedPayload });
  logger.warn({ userId: subscription.userId.toString(), rzpSubId }, 'subscription.halted — plan reverted to free');
}
  ...
  try {
    ...
    const [user, subscription] = await Promise.all([
      ...
      Subscription.findOne({ userId, status: { $in: ['active', 'created'] } })
        .select('razorpaySubscriptionId planType billingCycle status currentPeriodStart currentPeriodEnd refunded cancelRequested cancelRequestedAt createdAt')
        .lean(),
    ]);
...
router.post('/cancel-subscription', requireAuth, async (req, res) => {
  try {
    ...
    if (!subscription) {
      return res.status(404).json({ error: 'NO_ACTIVE_SUBSCRIPTION', message: 'No active subscription found.' });
    }

    if (subscription.refunded) {
      return res.status(409).json({ error: 'ALREADY_REFUNDED', message: 'This subscription was already refunded.' });
    }
    ...
    logger.info(
      { userId, subscriptionId: subscription.razorpaySubscriptionId, periodEnd: subscription.currentPeriodEnd },
      'User requested subscription cancellation (at cycle end)'
    );

    return res.json({
      ok:          true,
      message:     'Cancellation scheduled. You retain full access until the end of your current billing period.',
      accessUntil: subscription.currentPeriodEnd ?? null,
    });
