...
const OnboardingWizard = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetchWithCSRF(getApiUrl('/api/auth/complete-onboarding'), {
                method: 'POST',
            ...
            // Handle Bar Council Number conflict
            if (data.errorCode === 'BAR_COUNCIL_EXISTS') {
                setErrors({ barCouncilNumber: data.message });
                setCurrentStep(1); // Go back to identity step
                toast({
                    title: 'Bar Council Number Already Registered',
